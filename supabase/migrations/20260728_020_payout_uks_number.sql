-- ============================================================
-- 020 — Wypłata: opcjonalny numer UKS przy zleceniu
-- ============================================================
-- Komisant przy zaznaczeniu pozycji do wypłaty może rozliczyć się
-- UKS-em zamiast fakturą: wpisuje numer wygenerowanej/podpisanej
-- umowy, a numer ląduje w payouts.notes (widoczny dla admina).
-- Stara 2-argumentowa sygnatura znika (ambiguity w PostgREST).

drop function if exists public.request_payout_for_products(uuid[], uuid);

create or replace function public.request_payout_for_products(
  product_ids uuid[],
  bank_account uuid,
  uks_number text default null
)
returns uuid
language plpgsql
security definer
...
begin
  ...
  values (uid, total, bank_account, 'requested',
          ...
          || coalesce(' · UKS: ' || nullif(trim(uks_number), ''), ''))
...
revoke all on function public.request_payout_for_products(uuid[], uuid, text) from public;
grant execute on function public.request_payout_for_products(uuid[], uuid, text) to authenticated;
